...
render = |state| Gui.col(
	...
	[
		...
		if state.bytes.is_empty() {
			Gui.text("Choose an encoded image size")
		} else {
			Gui.image({ label: "Scaled image", bytes: state.bytes, format: Svg, width: Fill, height: Fill, grow: True })
		},
	],
)

main : Gui.Program(State)
main = Gui.run({ init: |_access| { bytes: [] }, render, window: { title: "Image benchmark", width: 1000, height: 700 } })
