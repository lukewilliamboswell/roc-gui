...
create_grid = |count, first_identity| {
	...
	for _ in List.repeat({}, count) {
		$cells = Index.set($cells, $id, { id: $id, key: Key.id(first_identity + $id - 1), pressed: False, presses: 0 })
		$id = $id + 1
	}
	{ cells: $cells, count, columns: if count == 100 10 else 100, next_identity: first_identity + count }
}

press : Cell -> Action(Cell)
press = |cell| Action.update({ ..cell, pressed: !cell.pressed, presses: cell.presses + 1 })
...
render = |state| {
	...
	Elem.col(
		...
		[
			Elem.text("Repeatedly click one cell without moving the pointer"),
			Elem.row(
				{ gap: 8 },
				[
...
main : Program(State)
main = Program.run({
	init: |_access| create_grid(10000, 1),
	render,
