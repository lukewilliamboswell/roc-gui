...
render = |state| Gui.col(
	{},
	[
		Gui.button({ caption: "Load 100 rows", label: "Load 100 rows", on_press: |_, _| Gui.Action.update({ rows: make_rows(100) }) }),
		Gui.button({ caption: "Load 1,000 rows", label: "Load 1000 rows", on_press: |_, _| Gui.Action.update({ rows: make_rows(1000) }) }),
		Gui.button({ caption: "Load 10,000 rows", label: "Load 10000 rows", on_press: |_, _| Gui.Action.update({ rows: make_rows(10000) }) }),
