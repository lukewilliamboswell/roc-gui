...
impl App {
    ...
    pub(crate) fn detect_accessed_entities<R>(
        ...
        // Track this scope independently: an entity read before this scope is
        // still one of its dependencies when it is read again inside it.
        // Moving the outer set also avoids copying every entity accessed by
        // the frame for each cached view that needs to be rebuilt.
        let outer = std::mem::take(&mut *self.entities.accessed_entities.borrow_mut());
        let result = callback(self);
        let accessed = std::mem::replace(&mut *self.entities.accessed_entities.borrow_mut(), outer);
...
mod test {
    ...
    #[test]
    fn accessed_entity_scope_includes_overlapping_dependencies() {
        let cx = TestAppContext::single();
        cx.update(|cx| {
            let already_read = cx.new(|_| 1_u8);
            let newly_read = cx.new(|_| 2_u8);
            let unrelated = cx.new(|_| 3_u8);
            cx.entities.clear_accessed();
            already_read.read(cx);
            unrelated.read(cx);
            let (value, accessed) =
                cx.detect_accessed_entities(|cx| *already_read.read(cx) + *newly_read.read(cx));
            assert_eq!(value, 3);
            assert_eq!(
                accessed,
                [already_read.entity_id(), newly_read.entity_id()]
                    .into_iter()
                    .collect()
            );
            assert_eq!(
                *cx.entities.accessed_entities.borrow(),
                [
                    already_read.entity_id(),
                    newly_read.entity_id(),
                    unrelated.entity_id()
                ]
                .into_iter()
                .collect()
    ...
    #[test]
    fn accessed_entity_scopes_nest_and_preserve_outer_dependencies() {
        let cx = TestAppContext::single();
        cx.update(|cx| {
            let before = cx.new(|_| 0_u8);
            let shared = cx.new(|_| 1_u8);
            let inside = cx.new(|_| 2_u8);
            let after = cx.new(|_| 3_u8);
            cx.entities.clear_accessed();
            before.read(cx);
            let (nested, outer) = cx.detect_accessed_entities(|cx| {
                shared.read(cx);
                let (_, nested) = cx.detect_accessed_entities(|cx| {
                    shared.read(cx);
                    inside.read(cx);
                });
                after.read(cx);
            ...
            assert_eq!(
                nested,
                [shared.entity_id(), inside.entity_id()]
                    .into_iter()
                    .collect()
            );
            assert_eq!(
                outer,
                [shared.entity_id(), inside.entity_id(), after.entity_id()]
                    .into_iter()
                    .collect()
            );
            assert_eq!(
            ...
            let (_, empty) = cx.detect_accessed_entities(|_| ());
