...
| Read project/files | `Files.pick_directory!`, `Dir.list!/open_read_dir!/read!`; `roc_files_*`; `files.rs` | `cap_std::ambient_authority` only when host config opens `--host-cap-dir`; fixture metadata provisions same registry | Root read handle; single-name children derive independent handles; refcount cleanup only, no ancestry or revocation | 10,000 entries, 4 MiB names, 64 MiB file; final symlinks not followed | File Explorer semantics/counters/scale and name unit test; trusted chooser, sibling denial, root/child revocation and races **Unverified** |
