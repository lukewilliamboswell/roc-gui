...
| SQLite | `Sqlite.open_read!/query!`; `roc_sqlite_*`; `sqlite.rs` | Derived from `DirRead`; bytes copied through no-follow child open | Independent in-memory read-only snapshot recorded as a grant derived from the directory's, so revoking the project revokes the snapshot; handle refcount cleanup is release, not revocation | 64 MiB DB, 64 KiB SQL, 256 columns, 10,000 rows, 16 MiB values; authorizer denies writes/attach | Database Browser error/counter/scale specs and `revoked-project.scm`, which revokes the project and requires the next query to be refused while the rows already returned stay on screen; extension/function escape audit **Unverified** |
