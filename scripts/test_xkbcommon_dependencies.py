"""Source identity and publication failures must not admit substitute Xkbcommon bytes."""
...
import build_xkbcommon as producer
from dependency_archive import digest, write_archive
import release_dependencies


class XkbcommonDependencyTests(unittest.TestCase):
    def setUp(self):
    ...
    def make_archive(self, missing_license=False, missing_x11=False):
        policy = release_dependencies.KINDS["xkbcommon"]
        files = {"targets/x64glibc/" + name: b"tested shared library" for name in policy["files"]}
        for name in policy["licenses"]:
            files["licenses/xkbcommon/" + name] = b"license text"
        if missing_license:
            del files["licenses/xkbcommon/LICENSE"]
        if missing_x11:
            del files["targets/x64glibc/libxkbcommon-x11.so"]
        return write_archive(self.root / producer.ARCHIVE_NAME,
                             {"schema_version": 1, "name": "xkbcommon", "version": "test",
    ...
    def prepare(self):
        ...
        with patch.object(release_dependencies.subprocess, "check_output", return_value="a" * 40), patch.object(
            ...
            release_dependencies.prepare(self.root, "deps-xkbcommon-1", environment, "xkbcommon")
        return verifier

    def test_release_lock_binds_the_xkbcommon_producer(self):
        archive = self.make_archive()
        verifier = self.prepare()
        lock = json.loads((self.root / "dependencies.lock.json").read_text())
        entry = lock["artifacts"]["xkbcommon-x64glibc"]
        self.assertTrue(entry["signer_workflow"].endswith("/.github/workflows/xkbcommon-dependencies.yml"))
    ...
    def test_missing_x11_library_refuses_publication(self):
        self.make_archive(missing_x11=True)
        with self.assertRaisesRegex(ValueError, "incomplete or unexpected"):
            self.prepare()
        self.assertFalse((self.root / "dependencies.lock.json").exists())
