-- Process-wide resources sampled outside the measured operation timing boundary.
-- Current RSS snapshots remain raw capture evidence, but are intentionally not
-- differenced here: allocator/page-release activity makes run-boundary deltas
-- unsuitable as a run-level resource measurement.
WITH evidence AS (
