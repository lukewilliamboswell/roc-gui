...
need target headers. SDK-dependent macOS checks run on macOS. Build scripts
...
def unavailable(triple: str) -> str | None:
    ...
    if triple.endswith("apple-darwin") and platform.system() != "Darwin":
        return "SDK-dependent macOS checks run on a Mac; Apple SDKs are not provisioned on this host"
