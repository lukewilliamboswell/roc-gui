...
Roc GUI generates the minimal text-based linker interfaces needed to connect its
compiled host to libraries supplied by macOS. The generated `.tbd` files are our
own output. They are produced from the reviewed [`interfaces.json`](interfaces.json)
...
- Community-maintained FFI bindings and GPUI source may explain why the host
  references an interface, but do not by themselves prove which system library
  owns it.
- The project-built `libhost.a` may be inspected only to inventory its unresolved
...
`scripts/audit_macos_archive.py` inventories unresolved references in the
project-built `libhost.a`. The inventory is intentionally broader than the final
...
packages that output as the independent external-input release; `libhost.a` is
not part of the interface archive's identity.
...
The generated files contain only the target, install names, reexports, and
selected symbol names required for linking. macOS supplies all implementations
at runtime.

## Reviewing a catalog change

1. Build `libhost.a` solely from Roc GUI and its reviewed open-source inputs.
2. Inventory unresolved references with `audit_macos_archive.py` using the LLVM
   reader from the Rust toolchain that built the archive.
...
5. Generate candidate interfaces offline and run the generator and archive-audit
   tests.
6. Final-link every maintained Roc GUI application with the selected host and
   candidate interfaces, then run its semantic specifications and native GPUI
   smoke test.
