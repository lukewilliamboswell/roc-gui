...
View := [].{
	render : Monitor.State -> Elem.Elem(Monitor.State)
	render = render
}
...
control = |state| match state.run_state {
	Running(session) => Elem.action_button(
		Elem.ActionButtonProps.{
			...
			font_weight: 600,
			bg: Theme.raised,
			hover_bg: Gui.rgb(0x24404b),
			active_bg: Gui.rgb(0x16272e),
			fg: Theme.ink,
			border_color: Theme.hairline,
			border_width: 1,
		},
	)
	Paused => Elem.action_button(
		Elem.ActionButtonProps.{
...
mark = Elem.image(Elem.ImageProps.{ label: "Attention mark", bytes: alert_icon, format: Svg, width: Px(18), height: Px(18) })
...
status_strip = |state| {
	...
	Elem.row(
		Elem.RowProps.{
			...
			align: Center,
			overflow_y: Clip,
			bg: if alarmed Gui.rgb(0x261a18) else Theme.surface,
			border_color: if alarmed Gui.rgb(0x5a3a33) else Theme.hairline,
			border_width: 1,
		},
...
tile_tone = |level| match level {
	...
	Missing => { bg: Gui.rgb(0x101c22), detail: Theme.absent, rule: Theme.absent, value: Theme.absent }
...
tile = |reading| {
	...
	Elem.col(
		...
		[
			...
			Elem.col(
				Elem.ColProps.{ width: Fill, padding: 14, gap: 6 },
				[
					Theme.caption(reading.caption),
					Elem.row(
						Elem.RowProps.{ padding: 0, gap: 5, align: Baseline },
						[
							Theme.figure(reading.value, 26, tone.value),
							Elem.row(
								Elem.RowProps.{ padding: 0, gap: 0, font_size: 12, font_weight: 700, fg: Theme.muted },
								[Elem.text(reading.unit)],
							),
						],
					),
					Elem.row(
						Elem.RowProps.{ padding: 0, gap: 0, font_size: 12, fg: tone.detail, text_overflow: Ellipsis },
						[Elem.text(reading.detail)],
					),
				],
			),
		],
...
readings = |state| Elem.row(
	Elem.RowProps.{ label: "Readings", width: Fill, padding: 0, gap: 12 },
	...
	while $position < held {
		...
		$items = match snapshot {
			Ok(value) => $items.append(
				Elem.VirtualListItem.{ key: $position, content: Theme.figure(Summary.history_line(value), 12, colour) },
			)
			Err(_) => $items
		}
		$position = $position + 1
...
sort_button = |caption, label, active, press| Elem.action_button(
	Elem.ActionButtonProps.{
		caption,
		label,
		on_press: press,
		...
		hover_bg: if active Theme.accent_tint else Gui.rgb(0x24404b),
		active_bg: Gui.rgb(0x16272e),
...
sort_controls = |state| Elem.row(
	Elem.RowProps.{ label: "Process sorting", padding: 0, gap: 8, align: Center },
	[
		Theme.caption("SORT"),
		sort_button("CPU", "Sort processes by CPU", state.sort == ByCpu, |current, _| Action.update({ ..current, sort: ByCpu })),
		sort_button("Memory", "Sort processes by memory", state.sort == ByMemory, |current, _| Action.update({ ..current, sort: ByMemory })),
	],
)

filter_field = |state| Elem.text_input(
	Elem.TextInputProps.{
		label: "Filter processes",
		value: state.filter,
		placeholder: "Filter by name",
		on_change: |current, event| Action.update({ ..current, filter: event.value }),
		on_submit: |current, _| Action.update(current),
		width: Fill,
		height: Px(34),
		font_size: 13,
		font_face: Monospace,
...
## nothing.
selection_strip = |state, processes| Elem.col(
	Elem.ColProps.{
		label: "Selection",
		width: Fill,
		height: Px(50),
...
process_row = |state, process| {
	...
	Elem.action_button(
		Elem.ActionButtonProps.{
			...
			on_press: |current, _| Action.update({ ..current, selected: Some(process.pid) }),
			width: Fill,
			height: Px(26),
			padding: 8,
			radius: 5,
			font_size: 12,
			font_face: Monospace,
			bg: if chosen Theme.accent_tint else Gui.rgb(0x0f1c23),
			hover_bg: Gui.rgb(0x1a2f38),
			active_bg: Gui.rgb(0x16272e),
			fg: if chosen Theme.accent else Theme.ink,
			border_color: if chosen Theme.accent else Gui.rgb(0x0f1c23),
			border_width: 2,
			border_top: Px(0),
			border_right: Px(0),
			border_bottom: Px(0),
...
process_table = |state, processes| {
	...
	[
		Elem.row(
			Elem.RowProps.{ width: Fill, padding: 0, gap: 0 },
...
render = |state| Elem.col(
	...
	[
		...
		Elem.row(
			...
			[
				Elem.col(
					Elem.ColProps.{ label: "Instruments", width: Fill, height: Fill, grow: True, padding: 0, gap: 12 },
