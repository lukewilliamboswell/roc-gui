...
State : { access : Gui.Access, back : List(Folder), dialog : Dialog, forward : List(Folder), read : Read, root : [None, Some(Folder)], selection : Selection, status : Status, view : View }
...
kind_mark = |kind, name| Gui.row(
	{ width: Px(30), padding: 0, gap: 0, justify: Center, align: Center },
	[Gui.image({ label: "${kind_name(kind)} marker ${name}", bytes: kind_art(kind), format: Svg, width: Px(15), height: Px(15) })],
)
...
## are actually on.
row_action = |caption, label, on_press| Gui.button({
	caption,
	label,
	on_press,
	width: Px(64),
...
entry_items = |state, folder| {
	...
	folder.entries.map_with_index(|entry, key| { key, content: entry_row(folder, entry, entry.name == selected_name) })
}

toolbar_button = |caption, label, enabled, on_press| Gui.button({
	caption,
	label,
	enabled,
	on_press,
...
busy_chip = Gui.row(
	{ label: "Loading status", padding: 6, padding_left: Px(10), padding_right: Px(10), gap: 0, bg: raised, fg: muted, font_size: 13, radius: 6 },
	[Gui.text("Loading…")],
)
...
error_panel = |failure| Gui.panel(
	{ label: "Directory error", width: Fill, padding: 14, gap: 8, bg: danger_bg, border_color: danger_edge },
	[
...
empty_state = |status| Gui.col(
	{ label: "No project", width: Fill, grow: True, gap: 10, padding: 32, align: Center, justify: Center },
	[
		Gui.image({ label: "No project marker", bytes: folder_icon, format: Svg, width: Px(40), height: Px(40) }),
		styled_text(
			match status {
	...
	read_lines = match state.read {
		...
		ReadOf(value) => if value.name != entry.name {
			...
			[
				styled_text("Read ${value.bytes.to_str()} bytes", accent, 13),
				Gui.row(
					{ width: Fill, padding: 8, gap: 0, bg: ground, fg: muted, font_size: 13, font_face: Monospace, radius: 5, text_overflow: Ellipsis, overflow_x: Clip },
					[Gui.text(value.preview)],
				),
			]
		}
	}
	Gui.panel(
		{ label: "Selection details", width: Fill, padding: 14, gap: 8, bg: surface, border_color: rule },
		[
			Gui.row(
...
render = |state| {
	...
	heading = Gui.row(
		{ label: "File explorer heading", width: Fill, gap: 16, align: Center, justify: Between },
		[
			Gui.col(
				{ gap: 3 },
				[
	...
	content = match state.view {
		Empty => Gui.panel(
			{ label: "Directory content", width: Fill, grow: True, padding: 0, gap: 0, bg: surface, border_color: rule, align: Center, overflow_y: Clip },
			[empty_state(state.status)],
		)
		Showing(folder) => Gui.panel(
			{ label: "Directory content", width: Fill, height: Fill, grow: True, padding: 12, gap: 10, bg: surface, border_color: rule, overflow_y: Clip },
			[
				Gui.row(
					{ label: "Directory toolbar", width: Fill, gap: 8, align: Center },
					[
	...
	Gui.col(
		{ label: "File explorer", width: Fill, height: Fill, grow: True, padding: 20, gap: 14, overflow_y: Clip },
		[heading]
