...
entry_items = |folder| folder.entries.map_with_index(|entry, key| {
	...
	select = Elem.action_button(Elem.ActionButtonProps.{ caption: entry.name, label: "Select ${label}", width: Px(320), justify: Start, on_press: |current, _| Action.update({ ..current, selection: Selected(entry), status: Ready }) })
