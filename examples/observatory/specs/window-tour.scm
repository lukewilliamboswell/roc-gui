...
(test "the window presents the capture list and every view of a capture"
  ...
  (steps
    ...
    (scroll (role scroll :name "Inspector scroll") :to (role row :name "Sum check"))
    (settle)
    (expect-on-screen (role row :name "Sum check"))
    (screenshot "waterfall")
    (scroll (role scroll :name "Inspector scroll") :to (role row :name "Allocation platform_lowering"))
    (settle)
