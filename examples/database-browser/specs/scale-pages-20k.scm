...
(test "turn to the last page of 20000 loans"
  ...
  (steps (click (role button :name "Choose database folder")) (await-task) (click (role button :name "Open database bookstore.db")) (await-task) (replace-text (role textarea :name "SQL query") "SELECT * FROM loans WHERE id <= 20000 ORDER BY id") (click (role button :name "Run query")) (await-task) (mark-metrics) (click (role button :name "Next page")) (await-task) (expect-visible (text "Rows 10001–20000, end of result")) (expect-count (text-prefix "Result row ") 10000)))
