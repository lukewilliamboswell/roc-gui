(test "browse 1000 database rows"
  ...
  (steps (click (role button :name "Choose database folder")) (await-task) (click (role button :name "Open database bookstore.db")) (await-task) (replace-text (role textarea :name "SQL query") "SELECT * FROM books LIMIT 1000") (mark-metrics) (click (role button :name "Run query")) (await-task) (expect-rows (role virtual-list :name "Query rows") :count 1000 :first 0 :mounted 64)))
