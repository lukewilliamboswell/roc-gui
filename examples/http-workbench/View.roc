...
request_side = |state| Gui.col(
	...
	[
		...
		field_row(
			"METHOD",
			[
				input({ label: "HTTP method", value: state.method, placeholder: "POST", on_change: |current, event| Gui.Action.update(Workbench.set_method(current, event.value)), width: Px(96), grow: False }),
				trailing_meta("${state.request.to_utf8().len().to_str()} B body"),
			],
		),
		field_row("URL", [input({ label: "Request URL", value: state.url, placeholder: "http://127.0.0.1:38191/echo", on_change: |current, event| Gui.Action.update(Workbench.set_url(current, event.value)), width: Fill, grow: True })]),
		field_row("QUERY", [input({ label: "Query parameters", value: state.query, placeholder: "page=1&limit=100", on_change: |current, event| Gui.Action.update(Workbench.set_query(current, event.value)), width: Fill, grow: True })]),
		field_row(
			"HEADER",
			[
				input({ label: "Header name", value: state.header_name, placeholder: "accept", on_change: |current, event| Gui.Action.update(Workbench.set_header_name(current, event.value)), width: Px(156), grow: False }),
				input({ label: "Header value", value: state.header_value, placeholder: "application/json", on_change: |current, event| Gui.Action.update(Workbench.set_header_value(current, event.value)), width: Fill, grow: True }),
			],
		),
		meta("BODY"),
		Gui.textarea({
			...
			on_input: |current, event| Gui.Action.update(Workbench.set_request(current, event.value)),
			width: Fill,
			height: Fill,
			grow: True,
			padding: Theme.inset,
