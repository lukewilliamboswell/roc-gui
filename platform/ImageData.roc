...
ImageData := [].{
	...
	encode_format = |format| match format { Bmp => 0, Gif => 1, Jpeg => 2, Png => 3, Svg => 4, Tiff => 5, Webp => 6 }
	decode_reason = |code| match code { 0 => Corrupt, 1 => InvalidDimensions, 2 => ResourceLimit, _ => Unsupported }
}
